  ...
  const Achieve = {
    s: {
      ...
      _stats: {
        ...
        maxScoreInGame: 0, // ✅ Thêm theo dõi điểm cao nhất trong ván
      },
    },

    list: [
      {id:'first_win',     name:'Bước khởi đầu',      desc:'Thắng ván đầu tiên',               lv:'🟢 Dễ',    check:()=>Achieve.s._stats.totalWin >=1},
      {id:'score_10',      name:'Điểm khởi đầu',       desc:'Ghi 10 điểm trong 1 ván',           lv:'🟢 Dễ',    check:()=>Achieve.s._stats.maxScoreInGame >=10}, // ✅ SỬA
    ...
    init(refs){
      ...
      this._load();
      this._buildUI();
      console.log('🏆 achievements.js — Đã sửa check() + score_10');
    },
    ...
    onScore(who, combo, pScore, aScore, ballSpeed){
      ...
      // ✅ Cập nhật điểm cao nhất trong ván
      if(pScore > this.s._stats.maxScoreInGame) this.s._stats.maxScoreInGame = pScore;
    ...
    onWin(who, diff, pScore, aScore, dur, hadShort, hadBomb, hadClone){
      ...
      // ✅ Đảm bảo maxScoreInGame được cập nhật cuối ván
      if(pScore > this.s._stats.maxScoreInGame) this.s._stats.maxScoreInGame = pScore;
      ...
      // ✅ Reset điểm cao nhất cho ván sau
      setTimeout(()=>{ this.s._stats.maxScoreInGame = 0; }, 100);
      this._save();
    },
    ...
    onLose(who){
      ...
      this.s._stats.maxScoreInGame = 0; // ✅ Reset cả khi thua
      this._save();
    },
