...
//  Hoàn toàn độc lập, không đụng items.js cũ
// ============================================================
(function () {
    'use strict';
    ...
    const Gacha = {
      s: {
        ...
        box: null,        // Hộp gacha đang rơi
        active: [],       // Hiệu ứng đang chạy
        player: null, ai: null, ball: null,
        pScore: null, aScore: null,  // Refs đến biến điểm
        _t: 0,            // Timer tự thả
      },
      ...
      _pool: [
        ...
        { id:'g_long',    good:true,  rarity:1, name:'VỢT DÀI +25%', color:'#22c55e', dur:8000,
          apply:(s)=>{ s.player.h = Math.round(s.player.h*1.25); },
          end:  (s)=>{ s.player.h = 80; } },
      ...
      init(ctx, W, H, refs){
        ...
        console.log('🎰 gacha.js đã sẵn sàng');
      },
  
      // ===== THẢ HỘP GACHA =====
      spawn(){
        if(this.s.box) return; // Chỉ 1 hộp 1 lúc
        this.s.box = {
      ...
      // ===== QUAY RANDOM THEO TỶ LỆ =====
      _roll(){
        const r = Math.random()*100;
        let rarity;
        if(r < 60) rarity = 1;      // Thường 60%
        else if(r < 90) rarity = 2; // Hiếm 30%
        else rarity = 3;            // Siêu hiếm 10%
        
        // 50% TỐT / 50% XẤU
        const wantGood = Math.random() > 0.5;
        const pool = this._pool.filter(e => e.rarity===rarity && e.good===wantGood);
        // Nếu không có thì lấy bất kỳ
      ...
      _apply(eff){
        ...
        if(window.Achieve) Achieve.onGachaOpen('player', eff.good, eff.rarity);
      },
