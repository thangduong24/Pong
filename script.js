(function () {
  ...
  const pl = {x:20, y:H/2-PH/2, w:PW, h:PH, baseH:PH}; // ✅ Thêm baseH
  const ai = {x:W-20-PW, y:H/2-PH/2, w:PW, h:PH, baseH:PH}; // ✅ Thêm baseH
  const b  = {x:W/2, y:H/2, r:8, vx:0, vy:0, sp:5.5, baseSp:5.5}; // ✅ Thêm baseSp
  ...
  function rb(dir){
    const f=Items?Items.getField():{W,H};W=f.W;H=f.H;
    ai.x=W-20-PW;
    pl.x=20; // ✅ Vợt bạn cũng di chuyển khi sân thay đổi → công bằng
  ...
  function rg(){
    ...
    // ✅ SỬA: chỉ reset về baseH nếu KHÔNG có hiệu ứng nào đang giữ chiều cao
    if(window.Items && Items.s.active.every(e=>e.id!=='paddleLen')) pl.h=PH;
    if(window.Items && Items.s.active.every(e=>e.id!=='paddleLen')) ai.h=PH;
    pl.y=Math.max(0,Math.min(H-pl.h,H/2-PH/2)); // ✅ Clamp vị trí sau khi đổi kích thước
    ai.y=Math.max(0,Math.min(H-ai.h,H/2-PH/2));
    b.r=8;b.sp=b.baseSp; // ✅ Luôn về tốc độ gốc
  ...
  function end(w){
    ...
    if(window.Achieve){
      ...
      Achieve.check(); // ✅ QUAN TRỌNG: kiểm tra thành tựu cuối ván
    }
  ...
  function up(ts){
    ...
    const cdActive = Items && Items.cdOn();
    if(cdActive){
      // ✅ SỬA: vẫn update gacha + vật phẩm rơi khi đếm ngược → không treo
      if(Items) Items.update(dt);
      if(Gacha) Gacha.update(dt);
      return;
    }
    if(!G.run)return;
    if(Items){Items.update(dt);const f=Items.getField();W=f.W;H=f.H;ai.x=W-20-PW;pl.x=20;}
    ...
    // AI — ✅SỬA: đỡ TẤT CẢ bóng (thật + nhân đôi) bay về phía mình
    const aS = Items ? Items.getSpeed(G.aiS[G.diff], 'ai') : G.aiS[G.diff];
    let targetBall = b;
    if(Items){
      const threats = [b, ...Items.s.clones.filter(c=>c.alive)].filter(bl=>bl.vx>0);
      if(threats.length>0) targetBall = threats.reduce((a,c)=>a.x>c.x?a:c); // đỡ bóng gần nhất
    }
    ai.y = AI.update(ai, targetBall, G.diff, H, aS);
    // ✅ Clamp vợt AI khi sân thu nhỏ → không ra ngoài
    ai.y = Math.max(0, Math.min(H-ai.h, ai.y));
    // Bóng + nhân đôi
    b.x+=b.vx;b.y+=b.vy;
    if(Items)Items.s.clones.forEach(cl=>{if(cl.alive){cl.x+=cl.vx;cl.y+=cl.vy;}});
    if(Items){
      // ✅ SỬA: ai ăn vật phẩm dựa trên VỊ TRÍ BÓNG, không phải vx
      const whoAte = b.x < W/2 ? 'player' : 'ai';
      ...
      if(picked && window.Achieve){
        ...
        Achieve.check(); // ✅ Kiểm tra thành tựu sau khi ăn vật phẩm
      }
    }
    ...
    if(b.x<-30){
      ...
      if(window.Achieve) { Achieve.onScore('ai', Items?Items.gb():0, G.pS, G.aS, Math.hypot(b.vx,b.vy)); Achieve.check(); }
    ...
    if(b.x>W+30){
      ...
      if(window.Achieve) { Achieve.onScore('player', Items?Items.gb():0, G.pS, G.aS, Math.hypot(b.vx,b.vy)); Achieve.check(); }
